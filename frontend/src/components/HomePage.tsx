...
const RecordingButton: React.FC<{ onStart: () => void, onStop: () => void, state: 'idle' | 'recording' | 'processing'}> = ({ onStart, onStop, state }) => {
    const isIdle = state === 'idle';
    const isRecording = state === 'recording';
    const isProcessing = state === 'processing';
    
    const buttonText = isIdle ? '开始录制' : isRecording ? '停止录制' : 'AI分析中...';
    ...
    return (
        <motion.button
          ...
          >
            {isRecording && <motion.div className="w-3 h-3 bg-white rounded-full mr-3" animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1, repeat: Infinity }} />}
            {isProcessing && (
